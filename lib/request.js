...
exports.send = (opt) => {
    return new Promise((fulfill, reject) => {
        ...
        request.on('response', (res) => {
            ...
            res.on('end', () => {
                try {
                    const ret = Buffer.concat(chunks, count).toString('utf8');
                    if (res.statusCode !== 200) console.log(ret);
                    fulfill(JSON.parse(ret));
                }
                catch (err) {
                    console.log(Buffer.concat(chunks, count).toString('utf8'));
                    reject({
                        lv: 'ERROR'
